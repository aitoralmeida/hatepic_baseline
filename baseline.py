...
def build_inceptionvresnetv2_model():
    # get the model without the denses
    base_model = inceptionresnetv2.InceptionResNetV2(weights='imagenet', include_top='false')
    new_dense = base_model.output
    # add the new denses to classify the hate images
    new_dense = Dense(1024, activation='relu')(new_dense)
    predictions = Dense(2, activation='softmax')(new_dense)
...
def get_training_data_inceptionresnetv2():    
    filenames = os.listdir(TRAIN_PATH) 
    shuffle(filenames)
    X_train = []
    y_train = []
    ...
    for filename in filenames:
        ...
        img = image.load_img(TRAIN_PATH+filename, target_size=(299, 299))
        x = image.img_to_array(img)
        x = inceptionresnetv2.preprocess_input(x)
        X_train.append(x)
    print 'Total train examples: ' + str(len(y_train))
    print 'No hate examples: ' + str(total_no_hate)
    print 'Hate examples: ' + str(total_hate)
...
def get_test_data_inceptionresnetv2(): 
    print '*****TEST*****'
    filenames = os.listdir(TEST_PATH) 
    shuffle(filenames)
    X_test = []
...
if __name__ == "__main__":
    ...
    sys.stdout.flush()
    print 'Building data...'
    X_train, y_train, X_val, y_val = get_training_data_vgg16()
    sys.stdout.flush()
    ...
    sys.stdout.flush()
    X_test, y_test = get_test_data_vgg16()
